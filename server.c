	...
	while(1){
		...
		for(i=0;i<total_clients;i++){
			...
			if(FD_ISSET(sd,&readfds)){
				...
				if(read_bytes <= 0){
				...
				else if(strcmp(start_token,"/activegroups") == 0){
					char rply[256];
					char temp[256];
					int check = 0;
				...
				else if(strcmp(start_token,"/activeallgroups") == 0){
					bzero(buf,sizeof(buf));
					strcat(buf,"Server:\n");
					char grp_id[256];
					int check = 0;
				...
				else if(strcmp(start_token,"/makegroupreq") == 0){
					char* token = strtok(buf, " "); 
					token = strtok(NULL," ");
					if(token == NULL){
						printf("INVALID format ..!\n");
						...
						char rply[1024];
						struct grp_create_req *grp_req = (struct grp_create_req*)malloc(sizeof(struct grp_create_req));
						grp_req->admin = sd;
						...
						while(token != NULL){
							...
							for(int j=0;j<total_clients;j++){
								if(clients[j] == client_id && client_id != sd){
									bzero(rply,sizeof(rply));
									sprintf(rply,"%05d want you to be in the group %05d..!\n",sd,grp_req->g_id);
					...
					if(grp_id > group_count || grp_id == 0){
						...
						if(check == 1){
							...
							for(int l=0;l<10;l++){
								if(grp_reqs[l] != NULL && grp_reqs[l]->g_id == grp_id){
									for(int k=0;k<grp_reqs[l]->no_of_clients;k++){
										if(grp_reqs[l]->members[k] == sd){
											...
											if(grp_reqs[l]->processed[k] == -8989){
												...
												//printf("inner rply %d clie %d\n",grp_reqs[l]->no_of_rplys,grp_reqs[l]->no_of_clients);
												
												grp_reqs[l]->dec_count++;
												sprintf(rply,"You are request for declaining to addig in group %05d is processed..!\n",grp_id);
												send(sd,rply,sizeof(rply),0);
												bzero(rply,sizeof(rply));
												sprintf(rply,"%05d is declined your request..!\n",sd);
												printf("%s",rply);
												send(grp_reqs[l]->admin,rply,sizeof(rply),0);
											}
											check =0;
									...
									//printf("rply %d clie %d dec %d\n",grp_reqs[l]->no_of_rplys,grp_reqs[l]->no_of_clients,grp_reqs[l]->dec_count);
						...
						if(check == 1){
								bzero(rply,sizeof(rply));
								sprintf(rply,"You donn't have any request to declain the group %05d\n",grp_id);
								send(sd,rply,sizeof(rply),0);
						}
					}
				}
